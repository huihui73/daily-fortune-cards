...
function getElementsAdvice(elements) {
  ...
  if (elements.wood < 0) advice.push("木弱：多吃绿色蔬菜，接触自然，养肝护肝");
  if (elements.fire < 0) advice.push("火弱：多吃红色食物，适当运动，养心安神");
  if (elements.earth < 0) advice.push("土弱：多吃黄色食物，保持规律作息，健脾养胃");
  if (elements.metal < 0) advice.push("金弱：多吃白色食物，深呼吸锻炼，润肺养气");
  if (elements.water < 0) advice.push("水弱：多吃黑色食物，充足饮水，补肾固精");

  if (advice.length === 0) advice.push("五行平衡，继续保持良好生活习惯。");
...
// Main fortune generation
function generateFortune(birthday, date = null) {
...
// Display results
function displayFortune(birthday, gender = 'unspecified') {
  console.log('\n' + '='.repeat(60));
  console.log('📱 每日推算卡 - 推算结果演示');
  ...
  console.log(`   性别: ${gender === 'unspecified' ? '保密' : (gender === 'male' ? '男' : '女')}`);
  console.log(`   推算日期: ${fortune.date}`);

  console.log(`\n🔮 本日卦象`);
  console.log(`   ${fortune.hexagramName} (第${fortune.hexagramId + 1}卦)`);
  console.log(`   ${fortune.hexagramInterpretation}`);

  console.log(`\n🌟 生命路径`);
  console.log(`   生命路径数: ${fortune.lifePathNumber}`);
  console.log(`   ${fortune.lifePathDesc}`);

  console.log(`\n☯️ 五行平衡`);
  console.log(`   木 ${fortune.elements.wood} | 火 ${fortune.elements.fire} | 土 ${fortune.elements.earth} | 金 ${fortune.elements.metal} | 水 ${fortune.elements.water}`);
  const elementsAdvice = getElementsAdvice(fortune.elements);
  elementsAdvice.forEach(advice => console.log(`   • ${advice}`));

  console.log(`\n🍀 幸运要素`);
  console.log(`   幸运色: ${fortune.luckyColor}`);
  console.log(`   幸运数字: ${fortune.luckyNumber}`);

  console.log(`\n👔 穿衣灵感`);
  const clothingAdvice = getClothingAdvice(fortune.elements, fortune.date);
  clothingAdvice.forEach(advice => console.log(`   • ${advice}`));

  console.log(`\n🍲 饮食建议`);
  const dietAdvice = getDietAdvice(fortune.elements);
  dietAdvice.forEach(advice => console.log(`   • ${advice}`));

  console.log(`\n✅ 每日微任务`);
  const dailyTasks = generateDailyTasks(fortune.hexagramId, fortune.lifePathNumber);
  dailyTasks.forEach((task, i) => console.log(`   ${i + 1}. ${task}`));

  console.log(`\n📝 日记 Prompts`);
  const journalPrompts = generateJournalPrompts(fortune.hexagramId, fortune.lifePathNumber);
  journalPrompts.forEach((prompt, i) => console.log(`   思考 ${i + 1}: ${prompt}`));

  console.log(`\n💧 健康提醒`);
  console.log(`   记得多喝水，保持规律作息，适当运动。`);
  console.log(`   今日建议饮水量：1.5-2.5升`);

  console.log(`\n🔒 隐私说明`);
  console.log(`   本应用优先本地存储，推算结果仅基于您的生日。`);
  console.log(`   手机号仅用于身份识别，已脱敏处理。`);
...
if (require.main === module) {
  // Demo with different birthdays
  console.log('\n🎯 演示 1: 1990-01-01 (男性)');
  displayFortune('1990-01-01', 'male');

  console.log('\n🎯 演示 2: 1995-05-20 (女性)');
  displayFortune('1995-05-20', 'female');

  console.log('\n🎯 演示 3: 2000-12-25 (保密)');
  displayFortune('2000-12-25', 'unspecified');

  console.log('\n💡 提示: 在微信开发者工具中导入项目即可看到完整的卡片式 UI');
  console.log('   项目路径: /Users/gonghuihui/daily-fortune-cards/miniprogram\n');
}
